...
class Rexarm():
    def __init__(self, joints, gripper):
        ...
        self.gripper_open_pos = np.deg2rad(-80.0)
        ...
        self.angle_limits = np.array([
                            [-180, 180],# -180, 180
                            [-110, 110],# The joint coordinates display a value 90 degrees higher than the commanded value. Display limits are 0, 180
                            [-110, 110],# 90,-110
                            [-90, 180],# 90, -90
                            [-110, 110],
        ...
        self.speed = [0.2] * self.num_joints        # 0 to 1
        self.max_torque = [0.9] * self.num_joints   # 0 to 1
    ...
    def initialize(self):
        for joint in self.joints:
            ...
            joint.set_torque_limit(0.7)
            joint.set_speed(0.2)
        position = np.array([0,85,0,0,0,0])*D2R
        self.set_positions(position, update_now = True)
        if(self.gripper != 0):
            self.gripper.enable_torque()
            self.gripper.set_torque_limit(0.9)
            self.gripper.set_speed(0.9)
            self.open_gripper()
    ...
    def set_positions(self, joint_angles, update_now = True):
        ...
        for i,joint in enumerate(self.joints):
            ...
            if(update_now):
                joint.set_position(joint_angles[i], num_tries = 50)
